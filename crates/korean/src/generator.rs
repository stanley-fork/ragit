...
pub fn gen_fsm(debug: bool) {
    let rules = vec![
        ...
        (TermKind::Any, vec![vec!["의", "만", "도", "에", "에서", "에게", "로", "까지", "부터", "한테", "하고", "께"]]),
...
fn generate_code(states: Vec<State>, debug: bool) -> String {
    ...
    lines.push((0, String::from("// This code is auto-generated by generator.rs")));
    ...
    for (id, state) in states.iter().enumerate() {
        ...
        for (자모, state_id) in transitions.iter() {
            let new_state = &states[*state_id];

            lines.push((4, format!(
                "Some(자모::{자모:?}  /* {} */) => {} {} {},",
                char::from_u32(자모.unwrap_u16() as u32).unwrap(),
                "{",
                if new_state.transitions.is_empty() {
                    match new_state.terminations.get(0) {
                        Some(TermKind::Any) => { String::from(" break count - 1; ") },
                        Some(TermKind::Offset) => { String::from(" break count - 2; ") },
                        _ => { String::from(" break 0; ") },
                    }
                } else {
                    format!("curr_state = {state_id}; continue;")
                },
                "}",
            )));
        }
        ...
        if state.terminations.is_empty() {
            lines.push((4, String::from("_ => { break 0; },")));
        }
            ...
            match state.terminations[0] {
                TermKind::Any => {
                    lines.push((4, String::from("_ => { break count - 1; },")));
                },
                TermKind::Offset => {
                    lines.push((4, String::from("_ => { break count - 2; },")));
                },
                _ => unreachable!(),
            }
        }

        lines.push((3, String::from("},")));
    }
