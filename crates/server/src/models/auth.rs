...
pub async fn create_api_key(user_id: i32, name: &str, expire: DateTime<Utc>, pool: &PgPool) -> Result<String, Error> {
    let key = format!("rgs_{}", encode_base64(&(0..240).map(|_| rand::random::<u8>()).collect::<Vec<_>>()));
    crate::query!(
        "INSERT INTO api_key (api_key, name, user_id, expire) VALUES ($1, $2, $3, $4)",
        &key,
        name,
        user_id,
        expire,
...
pub async fn get_api_key_list(user_id: i32, pool: &PgPool) -> Result<Vec<ApiKey>, Error> {
    let rows = crate::query!(
        "SELECT api_key, name, expire FROM api_key WHERE user_id = $1",
        user_id,
    ).fetch_all(pool).await?;
...
pub async fn check_password(user_id: i32, password: &str, pool: &PgPool) -> Result<bool, Error> {
    let row = crate::query!(
